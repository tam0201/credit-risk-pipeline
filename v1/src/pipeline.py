...
def preprocess_data(data, preprocessor, config):
    ...
    if isinstance(train_data, tuple):
        X_train_preprocessed, y_train_resampled = train_data
    else:
        X_train_preprocessed = train_data
        y_train_resampled = y_train

    if isinstance(test_data, tuple):
        X_test_preprocessed, y_test = test_data
    else:
        X_test_preprocessed = test_data
    ...
    if scipy.sparse.issparse(X_test_preprocessed):
            X_test_preprocessed = pd.DataFrame.sparse.from_spmatrix(
    ...
    X_test_preprocessed.to_csv(processed_data_dir / 'X_test.csv', index=False)
