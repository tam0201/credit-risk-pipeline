...
class Preprocessor(object):
    def __init__(
        ...
        for item in num_features:
            self.dtype_dict[item] = "float32" # for cpu memory saving
        
        self.tgt_nume_cols = []
        self.encoder = ce.ordinal.OrdinalEncoder(cols=cat_features,handle_unknown='impute')
        self.save_cate_avgs = {}
    ...
    def fit_transform(self, df):
        ...
        for item in tqdm(self.cat_features):
        ...
        for item in tqdm(self.num_features):
        ...
        # rows = None
        # for item in tqdm(self.num_features + self.tgt_nume_cols):
        #     feats = df[item].values
        #     if rows is None:
